...
void run_animation_half_fill_spin(uint64_t refresh_rate_ms);
void run_animation_h_line_sequence(uint64_t refresh_rate_ms);
void app_main(void) {
  i2c_init();
  ssd1306_init();
  draw_fill(0x00); // all pixels off

  run_animation_h_line_sequence(200);
}
